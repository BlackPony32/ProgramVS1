...
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QLineEdit, QPushButton, QDesktopWidget, QTableWidgetItem, \
    QHeaderView, QTableWidget, QScrollArea, QVBoxLayout, QWidget, QMessageBox
import psycopg2
...
class _Watched_Series_List(MyWindowFormat):
    ...
    def update_database(self):
        try:
            ...
            for row in range(self.table_widget.rowCount()):
                ...
                cursor.execute(
                    "UPDATE Watched_Series_list SET series_name = %s, date_added = %s, season_quantity = %s, series_status = %s, "
    ...
    def delete_movie(self, unique_id):
        try:
            ...
            cursor.execute(
                "DELETE FROM Watched_Series_list WHERE unique_id = %s",
                (unique_id,)
            )
            conn.commit()
            cursor.close()
